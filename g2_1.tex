...
\fill[white] (S2) circle (0.07);
\foreach \ang in {0,45,90,135}{
    \draw[white, line width=0.6pt, opacity=0.8]
        ($(S2)+(\ang:0.08)$) -- ($(S2)+(\ang:0.28)$);
}
...
\node[text=photoncol, font=\scriptsize] at (9.0, 4.5) {$h\nu$};
\node[text=photoncol, font=\scriptsize] at (9.0, 4.0) {$h\nu$};
