...
\node[text=labelcol, font=\large\bfseries] at (8.0, 8.3)
    {Second-Order Coherence \textemdash\ Two Telescopes
     \textemdash\ $g^{(2)}(\tau,B)$};
...
% ════════════════════════════════════════════════════════
% 2. PHOTON BULLETS TO TWO TELESCOPES
% ════════════════════════════════════════════════════════
\coordinate (TEL1) at (11.0, 5.5);
\coordinate (TEL2) at (11.0, 3.0);

% Solid lines from S1 to telescope 1 aperture
...
% Solid lines from S1 to telescope 2 aperture
\draw[photoncol, line width=0.9pt, opacity=0.9]
...
% Solid lines from S2 to telescope 1 aperture
\draw[photoncol, line width=0.9pt, opacity=0.9]
    ($(S2)+(0.1, 0)$) -- ($(TEL1)+(0.0, 0.55)$);
\draw[photoncol, line width=0.9pt, opacity=0.9]
    ($(S2)+(0.1, 0)$) -- ($(TEL1)+(0.0, 0.0)$);
\draw[photoncol, line width=0.9pt, opacity=0.9]
    ($(S2)+(0.1, 0)$) -- ($(TEL1)+(0.0,-0.55)$);

% Solid lines from S2 to telescope 2 aperture
\draw[photoncol, line width=0.9pt, opacity=0.9]
    ($(S2)+(0.1, 0)$) -- ($(TEL2)+(0.0, 0.55)$);
\draw[photoncol, line width=0.9pt, opacity=0.9]
    ($(S2)+(0.1, 0)$) -- ($(TEL2)+(0.0, 0.0)$);
\draw[photoncol, line width=0.9pt, opacity=0.9]
    ($(S2)+(0.1, 0)$) -- ($(TEL2)+(0.0,-0.55)$);

% Photon bullets near telescope 1
\fill[photoncol] (9.0, 5.8) circle (0.10);
\fill[photoncol] (9.5, 5.5) circle (0.10);
\fill[photoncol] (9.0, 5.2) circle (0.10);

% Photon bullets near telescope 2
\fill[photoncol] (9.0, 3.3) circle (0.10);
\fill[photoncol] (9.5, 3.0) circle (0.10);
\fill[photoncol] (9.0, 2.7) circle (0.10);

% gamma labels on select bullets
\node[text=photoncol, font=\small] at (8.4, 5.8) {$\gamma$};
\node[text=photoncol, font=\small] at (8.4, 3.3) {$\gamma$};
...
\node[text=white, font=\small, right=2pt] at (11.8, 4.25) {$B$};

% ════════════════════════════════════════════════════════
% 4. INTENSITY TIME SERIES (showing beating pattern)
% ════════════════════════════════════════════════════════
% Frame for intensity traces
\draw[labelcol, line width=1pt] (4.5, 0.5) rectangle (9.5, 2.0);

% Time axis label
\node[text=labelcol, font=\small] at (4.2, 0.75) {$I(t)$};
\node[text=labelcol, font=\small] at (9.8, 0.3) {$t$};

% Intensity trace for TEL1 (label)
\node[text=source1col, font=\small] at (4.0, 1.75) {TEL1:};

% Oscillating intensity at TEL1 (simple sine-like pattern)
\draw[source1col, line width=1.2pt]
    (5.0, 1.5) -- (5.5, 1.8) -- (6.0, 1.5) -- (6.5, 1.2) -- (7.0, 1.5) -- (7.5, 1.8) -- (8.0, 1.5) -- (8.5, 1.2) -- (9.0, 1.5);

% Intensity trace for TEL2 (label)
\node[text=source2col, font=\small] at (4.0, 0.9) {TEL2:};

% Oscillating intensity at TEL2 (slightly out of phase, showing decorrelation)
\draw[source2col, line width=1.2pt]
    (5.0, 0.7) -- (5.3, 0.9) -- (5.6, 0.7) -- (5.9, 0.5) -- (6.3, 0.7) -- (6.7, 1.0) -- (7.0, 0.7) -- (7.4, 0.4) -- (7.8, 0.7) -- (8.2, 1.0) -- (8.6, 0.65) -- (9.0, 0.7);

% Annotation: correlation depends on angular separation
\node[text=labelcol, font=\tiny, align=center] at (7.0, 0.1) {Angular separation $\theta_s$ determines\\correlation timescale $\tau_{\text{coh}} \propto 1/\theta_s$};
